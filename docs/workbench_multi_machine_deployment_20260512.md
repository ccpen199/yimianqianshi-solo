...
路径类变量只写本机真实路径，不要把其他机器的 `/Users/...`、`/Applications/...` 或 `C:\...` 复制到当前机器。服务端会忽略明显属于其他系统的路径，并在 `/api/workbench-config` 的 `ignored_env_paths` 中返回原因，避免 stale `.env` 把 Workbench 绑死到另一台电脑。
...
# Optional. Leave blank unless auto-detection fails.
CODEX_CLI=
CODEX_CLI_PATH=
...
TRAE_PROJECTS_ROOT=/path/to/trae_projects
...
`CODEX_CLI` 或 `CODEX_CLI_PATH` 只在自动探测失败时才需要显式指定。未配置时会依次探测 PATH、nvm、npm global、Homebrew、Windows `%APPDATA%\npm` 等常见位置；Windows 会优先使用同目录无扩展 shim，再 fallback 到 `.cmd/.bat/.exe`，避免从 `cmd /k` 直接启动 `.ps1`。
